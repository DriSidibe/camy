...
tmp_dir = "app/static/videos"
video_to_read_src = None
...
def read_video():
    global video_to_read_src
    while True:
        success, frame = video_to_read_src.read()
        if not success:
            break
        else:
            ret, buffer = cv2.imencode('.jpg', frame)
...
def getVideoThumbnail(filename):
    ...
    ret, frame = cap.read()
    if ret:
        cv2.imwrite(f"{static_folder}/videos/thumbnails/{filename}.png", frame)
    else:
...
def video_listing():
    global static_folder, video_to_read_src
    if request.cookies.get('utilisateur_camera') != "drissa":
        return redirect(url_for('login'))
    video_to_read = None
    video_folder = f'{static_folder}/videos'
    ...
    video_paths = [f"{'/'.join(video_folder.split('/')[1:3])}/{video}" for video in videos]
    ...
    thumbnails = [f"{'/'.join(video_folder.split('/')[1:3])}/thumbnails/{os.path.splitext(video)[0]}.png" for video in videos]

    if f"{request.args.get('src')}.avi" in videos:
        video_to_read = request.args.get('src')
        video_to_read_src = cv2.VideoCapture(f"{video_folder}/{video_to_read}.avi")
    return render_template('video_listing.html', videos=zip(videos, video_paths, thumbnails), video_to_read=video_to_read)
